...
!
	subroutine trans
        ...
        integer :: edof(mdim), nen, idof, inttype
        
        
        ! Declare rest of variables
        integer :: n, e, i, j, output_dof
        real(wp) :: DD(neqn), DD_old(neqn), DDD(neqn), DDD_old(neqn)
        real(wp) :: MD(neqn), MD_old(neqn), MDD(neqn), MDD_old(neqn), MDDD(neqn), MDDD_old(neqn)
        real(wp) :: CD(neqn), CD_old(neqn), CDD(neqn), CDD_old(neqn), CDDD(neqn), CDDD_old(neqn)
        real(wp) :: KD(neqn), KD_old(neqn), KDD(neqn), KDD_old(neqn), KDDD(neqn), KDDD_old(neqn)
        real(wp) :: alpha_damp, beta_damp, beta, gamma, timestep, force_scale, omega_d
        logical :: lumped_mass, transient_force, matlab_output, boundary_c, plot_output, harmonic_force
		
		
		! User Parameters
        omega_d = 0.0173*2 !driving freqeuncy of force radianer
        alpha_damp = 0.00000000000	!
        beta_damp = 0.000000000000	!
        beta = 1.0_wp/4.0_wp		!
        gamma = 0.50000000000		!
        timestep = 0.0500000000000	!
		!timestep = 1.8059090000
		
		no_timestep = 30000			!
        force_scale = 0.001
        inttype = 3
        	!1: Explicit central difference
        	!2: Explicit half step
        	!3: Implicit Newmark method
        lumped_mass = .false.
        transient_force = .true.
        boundary_c = .false.
        harmonic_force = .false.
        matlab_output = .true.
        output_dof = 146*2
        plot_output = .false.
        
		
		print *, "Direct integration type: ", inttype
		print '(A,f20.4)',"Timestep:", timestep
        print *, 'no_timestep:',no_timestep 
        print *, 'alpha_damp:',alpha_damp
        print *, 'beta_damp:',beta_damp
        print *, 'gamma:',gamma
        ...
        select case (boundary_c)
          	case(.false.)
            	cmatrix = alpha_damp*mmatrix + beta_damp*kmatrix
         	case(.true.)
            	do e = 1, ne
                	! Find element dofs
                    nen = element(e)%numnode
		...
		select case (inttype)
          	case(1) !Explicit central difference
				transmatrix = 1.0_wp/(timestep**2.0_wp)*mmatrix + 1.0_wp/(2.0_wp*timestep)*cmatrix    	
         	case(2) !Explicit half step
                transmatrix = mmatrix*1.0_wp/(timestep**2.0_wp)   
            case(3) !Implicit Newmark method
            	transmatrix = 1.0_wp/(beta*timestep**2.0_wp)*mmatrix + gamma/(beta*timestep)*cmatrix + kmatrix
        end select
		...
		D = 0;  DDD = 0
        DD = P -matmul(kmatrix,D) -matmul(cmatrix,D)
        call solve(mmatrix,DD)
		MD = 0; MDD = 0; MDDD = 0
        KD = 0; KDD = 0; KDDD = 0
        CD = 0; CDD = 0; CDDD = 0
        ...
        do n = 1, no_timestep-1
            ...
            MD_old = MD
            CD_old = CD
            if (inttype == 3) then !Implicit Newmark method
              	MDD_old = MDD
                MDDD_old = MDDD
            	CDD_old = CDD
                CDDD_old = CDDD
			...
			if (harmonic_force) then
            	p = force_scale*sin(omega_d*timestep*n)    
            end if
    
            ! Build MD (multiplied elementwise)
            select case(lumped_mass)
            	case(.true.)
                	call mmul(d_all(:,n),MD,3)	!MD lump
                    if (inttype == 3) then
              			call mmul(DD,MDD,3)    	!MDD lump
             			call mmul(DDD,MDDD,3)  	!MDDD lump
           			end if
                case(.false.)
					call mmul(d_all(:,n),MD,2)	!MD cons
                    if (inttype == 3) then
              			call mmul(DD,MDD,2)    	!MDD cons
             			call mmul(DDD,MDDD,2)  	!MDDD cons
           			end if
            end select
            
            ! Build KD (multiplied elementwise)
            call mmul(d_all(:,n),KD,1)			!KD
            if (inttype == 3) then
              	call mmul(DD,KD,1)				!KDD
                call mmul(DDD,KD,1)				!KDDD
            end if
            ...
            select case (boundary_c)
                ...
                case(.false.)
                	CD = alpha_damp*MD + beta_damp*KD			!CD
                    if (inttype == 3) then !Implicit
              			CDD = alpha_damp*MDD + beta_damp*KDD	!CDD
                		CDDD = alpha_damp*MDDD + beta_damp*KDDD	!CDDD
            		end if
        	end select
            ...
            select case(inttype)
                ...
                case(3) !Implicit Newmark method
                	d_all(:,n+1) = P &
                    	+ 1.0_wp/(beta*timestep**2.0_wp)*MD + 1.0_wp/(beta*timestep)*MDD + (1.0_wp/(2.0_wp*beta)-1.0_wp)*MDDD &
                        + gamma/(beta*timestep)*CD + (gamma/beta-1.0_wp)*CDD + timestep*(gamma/(2.0_wp*beta)-1.0_wp)*CDDD
			end select
			...
			if (inttype == 3) then
            	DD_old = DD
            	DDD_old = DDD
            	DD = gamma/(beta*timestep) * (D_all(:,n+1)-D_all(:,n)) &
                     -(gamma/beta-1.0_wp) * DD_old &
                     -timestep*(gamma/(2.0_wp*beta)-1.0_wp) * DDD_old
                DDD = 1.0_wp/(beta*timestep**2.0_wp)*(D_all(:,n+1)-D_all(:,n)) &
                	  -1.0_wp/(beta*timestep**2.0_wp)*timestep*DD_old &
                	  -(1.0_wp/(2.0_wp*beta)-1.0_wp)*DDD_old
            end if
        ...
        if (matlab_output) then
            ...
            call increment_filename(outfile_path)
            open(10, file=trim(outfile_path))
            print *, "Saving to path:", outfile_path
            write(10,*) 'clear all;close all;F=false;T=true;'
            write(10,*) 'inttype =',inttype,";"
            write(10,*) 'alpha_damp =',alpha_damp,";"
            write(10,*) 'beta_damp =',beta_damp,";"
            write(10,*) 'beta =',beta,";"
